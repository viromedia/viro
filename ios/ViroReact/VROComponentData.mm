...
// NOTE: This class was originally copied from RTCComponentData.m.
// When updating React, it's important to ensure this class mimics what's in RTCComponentData.
// Update the following methods in this class each time:
//   1. createViewWithTag methods
//   2. propBlockForKey
//   3. setProp methods
//
// If you cut and paste those methods completely, replace the following React components with their Viro equivalants, which are listed below:
// RCTShadowView->VRTShadowView, RTCPropBock->VROPropBlock, RTCComponentProp->VROComponentProp, RTCComponent->VROComponent, RTCViewManager->ViroViewManager.

typedef void (^VROPropBlock)(id<VROComponent> view, id json);

@interface VROComponentProp : NSObject

@property (nonatomic, copy, readonly) NSString *type;
@property (nonatomic, copy) VROPropBlock propBlock;

@end

@implementation VROComponentProp
...
@implementation VROComponentData
{
  id<VROComponent> _defaultView;
  VRTShadowView *_defaultShadowView;
                              ...
                              bridge:(RCTBridge *)bridge
{
  if ((self = [super init])) {
    ...
    while (cls != [ViroViewManager class]) {
...
{
  //VA TODO: add assert to make sure this is called in main thread
  
    id<VROComponent> view = nil; //VA: Uncomment this...(id<VROComponent>)(props ? [_manager viewWithProps:props] : [_manager view]);
  view.reactTag = tag;
  if ([view isKindOfClass:[VRTView class]]) {
  }
...
- (VRTView *)createViewWithTag:(NSNumber *)tag
{
  ...
  VRTView *view = [self.manager view];
  view.reactTag = tag;
  return view;
}
...
{
    VRTShadowView *shadowView = nil; //[_manager shadowView];
  shadowView.reactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (ViroViewManager *)manager
{
    if (!_manager) {
...
- (VROPropBlock)propBlockForKey:(NSString *)name
                   inDictionary:(NSMutableDictionary<NSString *, VROPropBlock> *)propBlocks
{
  BOOL shadowView = (propBlocks == _shadowPropBlocks);
  VROPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    
    __weak VROComponentData *weakSelf = self;
    ...
    if ([keyPath isEqualToString:@"__custom__"]) {
      ...
      propBlock = ^(id<VROComponent> view, id json) {
        VROComponentData *strongSelf = weakSelf;
        if (!strongSelf) {
          return;
        }
        ...
        if (!json && !strongSelf->_defaultView) {
          ...
          //VA: UNcomment this..strongSelf->_defaultView = [strongSelf createViewWithTag:nil];
        }
      ...
      if (type == NSSelectorFromString(@"RCTBubblingEventBlock:") ||
        ...
        __weak ViroViewManager *weakManager = self.manager;
        setterBlock = ^(id target, id json) {
          __weak id<VROComponent> weakTarget = target;
...
#pragma clang diagnostic pop
        ...
        if (!typeSignature) {
          ...
          return ^(__unused id<VROComponent> view, __unused id json){};
        }
...
break; \
    ...
    if (RCT_DEBUG) {
      ...
      VROPropBlock unwrappedBlock = propBlock;
      propBlock = ^(id<VROComponent> view, id json) {
...
- (void)setProps:(NSDictionary<NSString *, id> *)props forView:(id<VROComponent>)view
{
  if (!view) {
    return;
  }
...
- (void)setProps:(NSDictionary<NSString *, id> *)props forShadowView:(VRTShadowView *)shadowView
{
  if (!shadowView) {
    return;
  }
